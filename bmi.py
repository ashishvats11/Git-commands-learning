...
# Now we will create a function to calculate the BMI and categorise based on the bmi

def bmi_index(name,weight:float,height:float):
    
    bmi = weight/(height)**2
    if bmi<18:
        return f"Hi {name}, your bmi index is {bmi} and you're under weight"
    elif bmi > 18 and bmi < 24:
        return f"Hi {name}, your bmi index is {bmi} and you're normal weight"
    else: 
        return f"Hi {name}, your bmi index is {bmi} and you're OVER weight"
    
name = input("enter your name: ")
weight = float(input("Enter your weight: "))
height = float(input("Enter your height: "))
print(bmi_index(name,weight,height))